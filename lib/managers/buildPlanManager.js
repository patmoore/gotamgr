...
BuildPlanManagerType = ManagerType.create({
    callPrefix: 'buildPlanManager',
    ...
    extensions: {
        createBuildPlansBuildOrders: function (buildPlan, options) {
            this.createBuildOrders(buildPlan.buildDemands, buildPlan, buildPlan, options);
        },
        /**
         * Converts
         * @param buildRequest { storable1: quantity, ... storableN: quantity }
         * @param options { buildProcessors: }
         */
...
//                    terminals[storable] = true;
                    ...
                    buildPlan.addComponentBuildOrder(buildOrder);
                } else {
                    // assumes that the item is only built by 1 building.
                    var building;
                    var checkShop = _.contains(buildings_that_can, Buildings.shop);
                    var alwaysAvailableInShop;
                    if (!checkShop) {
                        building = buildings_that_can[0];
                        alwaysAvailableInShop = false;
                    } else {
                        alwaysAvailableInShop = Buildings.shop.builds[storable.dbCode].alwaysAvailable;
                        if (alwaysAvailableInShop) {
                            // if always available make user go buy.
                            building = Buildings.shop;
                        } else {
                            building = buildings_that_can[0];
                        }
                    }
                    ...
                    // determine actual buildTime
                    var initialBuildTime = howToBuild && typeof howToBuild.seconds === 'function' ? howToBuild['seconds']() : 0;
                    var buildTime = initialBuildTime;
                    if ( initialBuildTime > 0 ) {
                        var buildDiscountRate = _.reduce(options.buildProcessors, function (currentDiscountRate, buildProcessor) {
                            var returnedDiscountRate;
                            if (buildProcessor && _.isFunction(buildProcessor.getBuildDiscount)) {
                                returnedDiscountRate = buildProcessor.getBuildDiscount('seconds', storable, currentDiscountRate);
                            } else {
                                returnedDiscountRate = currentDiscountRate;
                            }
                            return returnedDiscountRate;
                        }, 0);
                        buildTime *= (100-buildDiscountRate) / 100;
                    }
                    ...
                    if (storable.resource) {
                        var buildOrder = new BuildOrder({
                            ...
                            quantity: quantity,
                            checkShop: checkShop,
                            alwaysAvailableInShop: alwaysAvailableInShop
                        });
                        buildOrders.push(buildOrder);
                        buildPlan.addComponentBuildOrder(buildOrder);
                    } else {
