...
BuildPlanManagerType = ManagerType.createSubClass('buildPlanManager',
    ...
    ],
    {
        ...
        currentPlayerBuildPlansCursor : function() {
            var thatManager = this.thatManager;
            var currentPlayer = PlayerManager.currentPlayerCursor(this.userId);
            if ( currentPlayer && !_.isEmpty(currentPlayer.buildPlanIds) ) {
                var buildPlans = BuildPlan.databaseTable.findById({$in: currentPlayer.buildPlanIds});
                return buildPlans;
            } else {
                return null;
            }
