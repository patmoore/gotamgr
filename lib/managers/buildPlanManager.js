...
BuildPlanManagerType = ManagerType.createSubClass('buildPlanManager',
    ...
    },
    {
        ...
        createBuildOrders: function(buildRequest, parentBuildOrder, buildPlan, options) {
            ...
            _.each(buildRequest, function(quantity, buildableKey, buildRequest){
                if ( quantity < 1 || buildableKey === 'seconds') {
                    // screen out items in the buildRequest that have zero quantity.
                    return;
                }
                var buildable = Buildables[buildableKey];
                var adventurable = Adventurable[buildableKey];
                // need to look for each building: we assume that each item can be built by only 1 building.
                var buildings_that_can = buildable && buildable.buildings?buildable.buildings:[];
...
//                    terminals[buildable] = true;
        ...
        needCalculate: function(buildOrder, inventory, depth) {
            ...
            var build_delta = {};
            _.each(buildOrder, function(quantity, buildable, buildRequest){
                if ( quantity < 1 ) {
                    // handle 0 quantities
                    return;
                }
