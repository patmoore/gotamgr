...
Meteor.startup(function(){
    function organize(skillGeneral, currentCampLevel, storable, quantity) {
        CampStorable[skillGeneral][currentCampLevel][storable] = quantity;
        if ( !(storable in StorablesByCamp)) {
            StorablesByCamp[storable] = {};
        }
        if ( !(skillGeneral in StorablesByCamp[storable])) {
            StorablesByCamp[storable][skillGeneral] = [];
        }
        StorablesByCamp[storable][skillGeneral][currentCampLevel] = quantity;
    }
    ...
    CampStorable[battle] = [];
    CampStorable[trade] = [];
    CampStorable[intrigue] = [];
    // i = the current camp level ( i.e. 0 = not built)
    var i;
    for(i = 0; i < 21; i++) {
        CampStorable[battle][i] = {};
        CampStorable[trade][i] = {}
        CampStorable[intrigue][i] = {};
        switch(i) {
...
//            organize(intrigue, i, Storables.fingerless_gloves, 30);
        ...
        Object.freeze(CampStorable[battle][i]);
        Object.freeze(CampStorable[trade][i]);
        Object.freeze(CampStorable[intrigue][i]);
    }
