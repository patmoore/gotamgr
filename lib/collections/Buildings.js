...
_.each(['stone','iron','cloth', 'fish','ore', 'horse','smallfolk', 'grains', 'fur', 'wood'], function(resource) {
    buildables[resource] = {
...
BuildableInBuildings = _.sortBy(
    _.filter(Buildables.symbols(), function(element) {
 ...
 */
getInventoryEnum= function(key) {
    var item = {};
    if ( key in Buildables) {
        _.extend(item, Buildables[key]);
    }
    if ( key in Adventurables) {
        _.extend(item, Adventurables[key]);
    }
    return item;
}
