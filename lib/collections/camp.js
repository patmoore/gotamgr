...
Camp = DbObjectType.create({
    ...
    properties: [
        ...
        // Garrison flag is used to mark camps that are left open for farming purposes.
        {
            ...
            garrisonSkillSpecialization: {
                jsonHelper: 'SkillSpecialization'
            }
        }
    ],
    databaseTableName: 'camp',
    privateProperties: {
        // return { storableDbCode: total needed }
        totalByStorable: {
            'get': function () {
                var totalled = {};
                for (var level = this.currentLevel; level < 21; level++) {
                    _.each(this.storableNeeds[level], function (quantity, storableDbCode) {
                        var storable = Storables.enumOf(storableDbCode);
                        totalled[storable.dbCode] = (totalled[storable.dbCode] || 0) + quantity;
                    });
                }
                return totalled;
            }
